...
public class DialogoAdicionarBano extends JDialog implements ActionListener
{
	...
	public void actionPerformed( ActionEvent pEvento )
	{
		...
		if( comando.equals( AGREGAR ) )
		{
			...
			String sanita = txtSanitarios.getText();
			String idCentroComercial = ((VOCentroComercial)cbCentrosComerciales.getSelectedItem()).getIdentificador();
			if( identificador.equals( "" ) || capacidadNormalStr.equals( "" ) || areaStr.equals( "" ) || sanita.equals( "" ) || idCentroComercial.equals( "" ) )
			{
			...
			else
			{
				try
				{
					...
					int sanitar = Integer.parseInt(sanita);
					long area = Long.parseLong(areaStr);
					if ( interfaz.buscarAreaPorValor(area) == null)
					{
						interfaz.adicionarArea();
					}
					interfaz.adicionarBano(identificador, capacidadNormal, area, sanitar, idCentroComercial, this);;
				}
				catch( NumberFormatException e2 )
				{
					JOptionPane.showMessageDialog( this, "La capacidad normal (int), el área (double) y el número de sanitarios (integer) deben ser números.", "Agregar Ascensor", JOptionPane.ERROR_MESSAGE );
				}
			}
		}
