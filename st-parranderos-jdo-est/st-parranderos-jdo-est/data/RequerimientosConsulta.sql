...
SELECT IDCENTROCOMERCIAL, NUM_VISITAS/AFOROTOTAL AS INDICE
FROM
(
...
SELECT IDCENTROCOMERCIAL, FECHA, NUM_VISITAS/AFOROTOTAL AS INDICE
FROM
(
    SELECT LECTOR.IDCENTROCOMERCIAL, FECHA, COUNT(*) AS NUM_VISITAS
    FROM REGISTRANCARNET JOIN LECTOR
    ON REGISTRANCARNET.IDLECTOR = LECTOR.ID
    WHERE FECHA BETWEEN TO_DATE('2018/05/19','yyyy/mm/dd') AND TO_DATE('2020/07/20','yyyy/mm/dd') AND LECTOR.IDCENTROCOMERCIAL = 6
    GROUP BY LECTOR.IDCENTROCOMERCIAL, FECHA
),
(
    SELECT SUM(AFORO) AS AFOROTOTAL
    FROM
...
SELECT LOCALCOMERCIAL.IDENTIFICADOR, NUM_VISITAS/AREA.AFORO AS INDICE
FROM LOCALCOMERCIAL
JOIN
    (
...
SELECT LOCALCOMERCIAL.IDENTIFICADOR, FECHA, NUM_VISITAS/AREA.AFORO AS INDICE
FROM LOCALCOMERCIAL
JOIN
    (
        SELECT IDLOCALCOMERCIAL, FECHA, COUNT(*) AS NUM_VISITAS
        FROM REGISTRANCARNET JOIN LECTOR
        ON REGISTRANCARNET.IDLECTOR = LECTOR.ID
        WHERE FECHA BETWEEN TO_DATE('2019/05/19','yyyy/mm/dd') AND TO_DATE('2020/07/20','yyyy/mm/dd') AND LECTOR.IDLOCALCOMERCIAL = 'LC101'
        GROUP BY IDLOCALCOMERCIAL, FECHA
    ) AUX
ON LOCALCOMERCIAL.IDENTIFICADOR = AUX.IDLOCALCOMERCIAL
JOIN AREA
ON LOCALCOMERCIAL.AREA = AREA.ID;
...
JOIN
    (
        ...
        JOIN TIPOLOCAL
        ON LOCALCOMERCIAL.TIPOLOCAL = TIPOLOCAL.ID
        WHERE FECHA BETWEEN TO_DATE('2019/05/19','yyyy/mm/dd') AND TO_DATE('2020/07/20','yyyy/mm/dd') AND TIPOLOCAL.TIPO = 'Restaurante'
        GROUP BY TIPOLOCAL.ID, TIPOLOCAL.TIPO
    ) AUX
ON LOCALCOMERCIAL.TIPOLOCAL = AUX.ID
JOIN AREA
